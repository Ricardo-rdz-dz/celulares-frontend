  ...
  return (
    <div className="w-full max-w-[21cm] mx-auto bg-white text-black font-sans print:w-[21cm] print:h-[28cm]">
      ...
      <div className="border-2 border-black p-6 flex flex-col min-h-[25cm] relative">
        ...
        {/* ✨ DETALLE DEL PRODUCTO VENDIDO (Con Foto, IMEI y Color) */}
          ...
          <table className="w-full text-base font-mono">
            ...
            <tbody>
              <tr className="font-bold text-lg">
                ...
                <td className="pt-4 leading-tight flex gap-4 items-start">
                  
                  {/* Miniatura de la Foto */}
                  {venta.inventario?.imagen_url && (
                    <img 
                      src={venta.inventario.imagen_url} 
                      alt="Equipo" 
                      className="w-16 h-16 object-cover border border-gray-300 p-0.5 bg-white shadow-sm shrink-0" 
                    />
                  )}
                  ...
                  <div>
                    ...
                    <span className="text-xs font-normal text-gray-500">SKU: {venta.inventario?.sku || 'N/A'}</span>
                    
                    {/* Renderizamos datos extra si existen */}
                    <div className="mt-1 space-y-0.5 text-[11px] text-gray-600 font-normal leading-tight">
                      {venta.inventario?.color && <p>Color: <span className="font-bold uppercase text-gray-800">{venta.inventario.color}</span></p>}
                      {venta.inventario?.imei && <p>IMEI: <span className="font-bold text-gray-800">{venta.inventario.imei}</span></p>}
                      {venta.inventario?.numero_serie && <p>Serie: <span className="font-bold text-gray-800">{venta.inventario.numero_serie}</span></p>}
                    </div>
                  </div>
        ...
        <div className="text-base bg-gray-50 border border-gray-300 p-4 leading-tight flex justify-between items-center mb-2">
          <div>
            ...
            {/* ✨ Agregamos quién lo atendió */}
            <p className="text-[10px] text-gray-500 mt-2 uppercase font-bold tracking-wider">Atendido por: {venta.vendedor || 'Asesor MovilPlace'}</p>
          </div>
        ...
        <div className="mt-auto pt-6 border-t-2 border-dashed border-gray-300">
            <div className="grid grid-cols-3 gap-2">
              ...
              <div className="p-2 border border-black border-dashed rounded text-center flex flex-col items-center bg-gray-50">
                ...
                <p className="text-[9px] text-gray-500 mt-1">Únete a nuestro Canal Exclusivo WhatsApp </p>
              </div>
